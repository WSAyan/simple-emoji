...
class EmojiconHandler {
    ...
    companion object {
        private val sEmojisMap = SparseIntArray(1029)
        private val sSoftbanksMap = SparseIntArray(471)
        private val sEmojisModifiedMap = HashMap<String, Int>()
        private val sInexistentEmojis = HashSet<String>()
...
/// ELECT
        ...
        private fun isSoftBankEmoji(c: Char): Boolean {
            return c.toInt() shr 12 == 0xe
        }

        private fun getEmojiResource(context: Context, codePoint: Int): Int {
            return sEmojisMap.get(codePoint)
        }

        private fun getSoftbankEmojiResource(c: Char): Int {
            return sSoftbanksMap.get(c.toInt())
        }
         ...
         * @param textSize
         */
        fun addEmojis(context: Context, text: Spannable, emojiSize: Int, emojiAlignment: Int, textSize: Int) {
         ...
         * @param length
         */
        fun addEmojis(context: Context, text: Spannable, emojiSize: Int, emojiAlignment: Int, textSize: Int, index: Int, length: Int) {
         ...
         */
        fun addEmojis(context: Context, text: Spannable, emojiSize: Int, emojiAlignment: Int,
                      textSize: Int, useSystemDefault: Boolean) {
         ...
         * @param useSystemDefault
         */
        fun addEmojis(context: Context, text: Spannable, emojiSize: Int, emojiAlignment: Int, textSize: Int, index: Int, length: Int, useSystemDefault: Boolean) {
            ...
            val oldSpans = text.getSpans<EmojiconSpan>(0, textLength, EmojiconSpan::class.java)
            ...
            while (i < textLengthToProcess) {
                    ...
                    if (i + skip < textLengthToProcess) {
                        ...
                        if (followUnicode == 0xfe0f) {
                            ...
                            if (i + skip + followSkip < textLengthToProcess) {
                                ...
                                val nextFollowUnicode = Character.codePointAt(text, i + skip + followSkip)
                                if (nextFollowUnicode == 0x20e3) {
                                    var nextFollowSkip = Character.charCount(nextFollowUnicode)
                                    val tempIcon = getKeyCapEmoji(unicode)
                            ...
                            val hexUnicode = Integer.toHexString(unicode)
                            val hexFollowUnicode = Integer.toHexString(followUnicode)

                            val resourceName = "emoji_" + hexUnicode + "_" + hexFollowUnicode

                            var resourceId = 0
                            if (sEmojisModifiedMap.containsKey(resourceName)) {
                                resourceId = sEmojisModifiedMap[resourceName]?.toInt()!!
                            } else if (!sInexistentEmojis.contains(resourceName)) {
                                resourceId = context.resources.getIdentifier(resourceName, "drawable", context.applicationContext.packageName)
                                if (resourceId != 0) {
                ...
                if (icon > 0) {
                    text.setSpan(EmojiconSpan(context, icon, emojiSize, emojiAlignment, textSize), i, i + skip, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
                }
                i += skip
            }
        }

        private fun getKeyCapEmoji(unicode: Int): Int {
            var icon = 0
            when (unicode) {
                0x0023 -> icon = R.drawable.emoji_0023
                0x002a -> icon = R.drawable.emoji_002a_20e3
